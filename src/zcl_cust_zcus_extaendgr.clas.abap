CLASS zcl_cust_zcus_extaendgr DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS check_key
      IMPORTING
        !id_dienstleistung TYPE zcust_zcus_extaendgr-dienstleistung
        !id_ext_aendgrund  TYPE zcust_zcus_extaendgr-ext_aendgrund
      EXCEPTIONS
        not_found .
    CLASS-METHODS count
      RETURNING
    ...
    CLASS-METHODS get_detail
      ...
      RETURNING
        VALUE(rs_detail)   TYPE zcust_zcus_extaendgr
      EXCEPTIONS
        not_found .
    CLASS-METHODS get_detail_x
      IMPORTING
        ...
        !id_require        TYPE char1 DEFAULT ' '
      RETURNING
        VALUE(rs_detail_x) TYPE zcust_zcus_extaendgr_x
      EXCEPTIONS
        not_found .
    CLASS-METHODS get_list
      IMPORTING
        !id_require TYPE char1 DEFAULT ' '
      EXPORTING
        !et_list    TYPE z_t_zcus_extaendgr
      EXCEPTIONS
        not_found .
    CLASS-METHODS get_list_x
      IMPORTING
        ...
        !id_require TYPE char1 DEFAULT ' '
      EXPORTING
        !et_list_x  TYPE z_t_zcus_extaendgr_x
      EXCEPTIONS
        not_found .
    ...
    CLASS-METHODS get_text
      IMPORTING
        ...
        !id_require        TYPE char1 DEFAULT ' '
      RETURNING
        VALUE(rd_text)     TYPE zcust_zcus_extaendgr_x-aendgrund_bez
      EXCEPTIONS
        not_found .
    CLASS-METHODS reset_buffer .
    CLASS-METHODS transfer_to_buffer
      IMPORTING
        !it_list TYPE z_t_zcus_extaendgr .
PROTECTED SECTION.
PRIVATE SECTION.

  CONSTANTS mc_tabname TYPE tabname VALUE 'ZCUS_EXTAENDGR'. "#EC NOTEXT
  CONSTANTS mc_text_tabname TYPE tabname VALUE 'ZCUS_EXTAENDGRT'. "#EC NOTEXT
  CLASS-DATA mt_buffer TYPE z_t_zcus_extaendgr .
  CLASS-DATA ms_buffer_hl TYPE zcust_zcus_extaendgr .
  ...
  CLASS-DATA ms_full_buffer_hl TYPE zcust_zcus_extaendgr_x .
  ...
  CLASS-METHODS _complete_detail_x
    ...
    CHANGING
      !cs_detail_x TYPE zcust_zcus_extaendgr_x
    EXCEPTIONS
      not_found .
ENDCLASS.
...
CLASS ZCL_CUST_ZCUS_EXTAENDGR IMPLEMENTATION.
...
* headerline buffer usable?
  IF ( ms_buffer_hl-dienstleistung = id_dienstleistung ) AND
     ( ms_buffer_hl-ext_aendgrund = id_ext_aendgrund ) AND
...
*   search table buffer
    READ TABLE mt_buffer INTO ms_buffer_hl
               WITH KEY dienstleistung = id_dienstleistung
                        ext_aendgrund = id_ext_aendgrund
               BINARY SEARCH.
    IF sy-subrc = 0.
...
* text headerline buffer usable (complete data)?
  IF ( ms_full_buffer_hl-spras = id_langu ) AND
     ...
     ( ms_full_buffer_hl-ext_aendgrund = id_ext_aendgrund ) AND
     ( ms_full_buffer_hl-aendgrund_bez IS NOT INITIAL ).
...
*   entry found in headerline buffer
    rs_detail_x-zcust_zcus_extaendgr = ms_buffer_hl.
...
*   copy relevant set of entries and complete additional data
    LOOP AT mt_buffer ASSIGNING <ls_detail> FROM sy-tabix.
      ...
      ELSE.
        CLEAR ls_detail_x.
        ls_detail_x-zcust_zcus_extaendgr = <ls_detail>.
        CALL METHOD _complete_detail_x
          EXPORTING
            id_langu    = id_langu
            id_xsearch  = id_xsearch
...
METHOD get_text.
  ...
  DATA:
    ls_detail_x TYPE zcust_zcus_extaendgr_x.
...
*   entry found in text headerline buffer
    rd_text = ms_full_buffer_hl-aendgrund_bez.
    RETURN.
...
*   entry found in headerline buffer
    ls_detail_x-zcust_zcus_extaendgr = ms_buffer_hl.
...
* set result
  rd_text = ls_detail_x-aendgrund_bez.
...
* headerline buffer usable?
  IF ( ms_text_buffer_hl-spras = id_langu ) AND
     ...
     ( ms_text_buffer_hl-ext_aendgrund
                   = cs_detail_x-ext_aendgrund ) AND
...
*   search table buffer and put result into text headerline buffer
    READ TABLE mt_text_buffer INTO ms_text_buffer_hl
         WITH KEY spras = id_langu
                  ...
                  ext_aendgrund = cs_detail_x-ext_aendgrund
         BINARY SEARCH.
    IF sy-subrc = 0.
...
*     select from database
      ...
      CALL METHOD _select_single_text
        EXPORTING
          ...
          id_ext_aendgrund  = cs_detail_x-ext_aendgrund
        EXCEPTIONS
          not_found         = 1
          OTHERS            = 2.
      IF sy-subrc = 0.
...
* text required?
  IF ( id_require = 'T' ) AND
     ( cs_detail_x-aendgrund_bez IS INITIAL ).
    CONCATENATE cs_detail_x-dienstleistung
                cs_detail_x-ext_aendgrund
...
*   use concatenated key as text
    IF ( id_xsearch CA '13' ) AND
      ...
      CONCATENATE cs_detail_x-dienstleistung
                  cs_detail_x-ext_aendgrund
                  space              "for technical reason only
             INTO cs_detail_x-aendgrund_bez SEPARATED BY space.
      CONCATENATE '<' cs_detail_x-aendgrund_bez '>'
             INTO cs_detail_x-aendgrund_bez.
    ENDIF.
...
* read from database
  SELECT     *
    ...
    FROM     zcus_extaendgr.          "#EC CI_SGLSELECT "#EC CI_GENBUFF
...
*   read from database
    SELECT     SINGLE *
      ...
      WHERE  ( dienstleistung = id_dienstleistung )
        AND  ( ext_aendgrund = id_ext_aendgrund )
        .
    IF sy-subrc = 0.
*     insert into buffer
