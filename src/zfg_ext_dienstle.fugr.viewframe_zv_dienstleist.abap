*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZV_DIENSTLEIST
*---------------------------------------------------------------------*
FUNCTION viewframe_zv_dienstleist      .
...
*----------------------------------------------------------------------*
    CALL FUNCTION 'VIEWPROC_ZV_DIENSTLEIST'
      EXPORTING
        fcode                     = read
      ...
      TABLES
        ...
        extract                   = zv_dienstleist_extract
        total                     = zv_dienstleist_total
        x_header                  = x_header
        x_namtab                  = x_namtab
...
*----------------------------------------------------------------------*
      DO.
        CALL FUNCTION 'VIEWPROC_ZV_DIENSTLEIST'
          ...
          IMPORTING
            ucomm                     = function
            update_required           = status_zv_dienstleist-upd_flag
          TABLES
            excl_cua_funct            = excl_cua_funct
            extract                   = zv_dienstleist_extract
            total                     = zv_dienstleist_total
...
*----------------------------------------------------------------------*
      ...
      IF ( function EQ switch_to_show_mode OR
           ...
           function EQ end ) AND
status_zv_dienstleist-upd_flag NE space.
        PERFORM beenden.
        CASE sy-subrc.
          WHEN 0.
            CALL FUNCTION 'VIEWPROC_ZV_DIENSTLEIST'
            ...
            CASE sy-subrc.
              WHEN 0.
                IF status_zv_dienstleist-upd_flag EQ space. EXIT. ENDIF.
...
*----------------------------------------------------------------------*
      ELSEIF function EQ transport.
        IF status_zv_dienstleist-upd_flag NE space.
          ...
          CASE sy-subrc.
            WHEN 0.
              CALL FUNCTION 'VIEWPROC_ZV_DIENSTLEIST'
                ...
                IMPORTING
                  update_required           =
                                              status_zv_dienstleist-upd_flag
                TABLES
                  excl_cua_funct            = excl_cua_funct
                  extract                   = zv_dienstleist_extract
                  total                     = zv_dienstleist_total
