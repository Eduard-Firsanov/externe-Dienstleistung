*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZV_EXTLIEF
*---------------------------------------------------------------------*
FUNCTION viewframe_zv_extlief          .
...
*----------------------------------------------------------------------*
    CALL FUNCTION 'VIEWPROC_ZV_EXTLIEF'
      EXPORTING
        fcode                     = read
      ...
      TABLES
        ...
        extract                   = zv_extlief_extract
        total                     = zv_extlief_total
        x_header                  = x_header
        x_namtab                  = x_namtab
...
*----------------------------------------------------------------------*
      DO.
        CALL FUNCTION 'VIEWPROC_ZV_EXTLIEF'
          ...
          IMPORTING
            ucomm                     = function
            update_required           = status_zv_extlief-upd_flag
          TABLES
            excl_cua_funct            = excl_cua_funct
            extract                   = zv_extlief_extract
            total                     = zv_extlief_total
...
*----------------------------------------------------------------------*
      ...
      IF ( function EQ switch_to_show_mode OR
           ...
           function EQ end ) AND
status_zv_extlief-upd_flag NE space.
        PERFORM beenden.
        CASE sy-subrc.
          WHEN 0.
            CALL FUNCTION 'VIEWPROC_ZV_EXTLIEF'
            ...
            CASE sy-subrc.
              WHEN 0.
                IF status_zv_extlief-upd_flag EQ space. EXIT. ENDIF.
...
*----------------------------------------------------------------------*
      ELSEIF function EQ transport.
        IF status_zv_extlief-upd_flag NE space.
