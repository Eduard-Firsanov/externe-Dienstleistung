CLASS zcl_cust_zcus_aktivit DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS check_key
      IMPORTING
        !id_aktivitaet TYPE zcust_zcus_aktivit-aktivitaet
      EXCEPTIONS
        not_found .
    CLASS-METHODS count
      RETURNING
    ...
    CLASS-METHODS get_detail
      ...
      RETURNING
        VALUE(rs_detail) TYPE zcust_zcus_aktivit
      EXCEPTIONS
        not_found .
    CLASS-METHODS get_detail_x
      IMPORTING
        ...
        !id_require        TYPE char1 DEFAULT ' '
      RETURNING
        VALUE(rs_detail_x) TYPE zcust_zcus_aktivit_x
      EXCEPTIONS
        not_found .
    ...
    CLASS-METHODS get_list_x
      IMPORTING
        ...
        !id_require TYPE char1 DEFAULT ' '
      EXPORTING
        !et_list_x  TYPE z_t_zcus_aktivit_x
      EXCEPTIONS
        not_found .
    CLASS-METHODS get_text
      IMPORTING
        ...
        !id_require    TYPE char1 DEFAULT ' '
      RETURNING
        VALUE(rd_text) TYPE zcust_zcus_aktivit_x-aktivitaetbez
      EXCEPTIONS
        not_found .
    CLASS-METHODS reset_buffer .
    CLASS-METHODS transfer_to_buffer
      IMPORTING
        !it_list TYPE z_t_zcus_aktivit .
PROTECTED SECTION.
PRIVATE SECTION.

  CONSTANTS mc_tabname TYPE tabname VALUE 'ZCUS_AKTIVIT'.   "#EC NOTEXT
  CONSTANTS mc_text_tabname TYPE tabname VALUE 'ZCUS_AKTIVITT'. "#EC NOTEXT
  CLASS-DATA mt_buffer TYPE z_t_zcus_aktivit .
  CLASS-DATA ms_buffer_hl TYPE zcust_zcus_aktivit .
  ...
  CLASS-DATA ms_full_buffer_hl TYPE zcust_zcus_aktivit_x .
  ...
  CLASS-METHODS _complete_detail_x
    ...
    CHANGING
      !cs_detail_x TYPE zcust_zcus_aktivit_x
    EXCEPTIONS
      not_found .
ENDCLASS.
...
CLASS ZCL_CUST_ZCUS_AKTIVIT IMPLEMENTATION.
...
* headerline buffer usable?
  IF ( ms_buffer_hl-aktivitaet = id_aktivitaet ) AND
...
*   search table buffer
    READ TABLE mt_buffer INTO ms_buffer_hl
               WITH KEY aktivitaet = id_aktivitaet
               BINARY SEARCH.
    IF sy-subrc = 0.
...
* text headerline buffer usable (complete data)?
  IF ( ms_full_buffer_hl-spras = id_langu ) AND
     ( ms_full_buffer_hl-aktivitaet = id_aktivitaet ) AND
     ( ms_full_buffer_hl-aktivitaetbez IS NOT INITIAL ).
...
*   entry found in headerline buffer
    rs_detail_x-zcust_zcus_aktivit = ms_buffer_hl.
...
* copy table buffer and complete additional data
  LOOP AT mt_buffer ASSIGNING <ls_detail>.
    CLEAR ls_detail_x.
    ls_detail_x-zcust_zcus_aktivit = <ls_detail>.
    CALL METHOD _complete_detail_x
      EXPORTING
        id_langu    = id_langu
        id_xsearch  = id_xsearch
...
METHOD get_text.
  ...
  DATA:
    ls_detail_x TYPE zcust_zcus_aktivit_x.
...
*   entry found in text headerline buffer
    rd_text = ms_full_buffer_hl-aktivitaetbez.
    RETURN.
...
*   entry found in headerline buffer
    ls_detail_x-zcust_zcus_aktivit = ms_buffer_hl.
...
* set result
  rd_text = ls_detail_x-aktivitaetbez.
...
* BODY
  ...
  IF it_list IS NOT INITIAL.
    ...
    ELSE.
      ...
      SORT lt_list_temp BY aktivitaet
                           .
      DELETE ADJACENT DUPLICATES FROM lt_list_temp
                 COMPARING aktivitaet
                           .
...
* headerline buffer usable?
  IF ( ms_text_buffer_hl-spras = id_langu ) AND
     ( ms_text_buffer_hl-aktivitaet
                   = cs_detail_x-aktivitaet ) AND
...
*   search table buffer and put result into text headerline buffer
    READ TABLE mt_text_buffer INTO ms_text_buffer_hl
         WITH KEY spras = id_langu
                  aktivitaet = cs_detail_x-aktivitaet
         BINARY SEARCH.
    IF sy-subrc = 0.
...
*     select from database
      ...
      CALL METHOD _select_single_text
        EXPORTING
          id_langu      = id_langu
          id_aktivitaet = cs_detail_x-aktivitaet
        EXCEPTIONS
          not_found     = 1
          OTHERS        = 2.
      IF sy-subrc = 0.
...
* text required?
  IF ( id_require = 'T' ) AND
     ( cs_detail_x-aktivitaetbez IS INITIAL ).
    CONCATENATE cs_detail_x-aktivitaet
...
*   use concatenated key as text
    IF ( id_xsearch CA '13' ) AND
      ...
      CONCATENATE cs_detail_x-aktivitaet
                  space              "for technical reason only
             INTO cs_detail_x-aktivitaetbez SEPARATED BY space.
      CONCATENATE '<' cs_detail_x-aktivitaetbez '>'
             INTO cs_detail_x-aktivitaetbez.
    ENDIF.
...
* read from database
  SELECT     *
    ...
    FROM     zcus_aktivit.            "#EC CI_SGLSELECT "#EC CI_GENBUFF

* sort separately (data source may be a view)
  SORT mt_buffer BY aktivitaet
                    .
  mf_buffer_complete = abap_true.
...
*   read from database
    SELECT     SINGLE *
      ...
      FROM     zcus_aktivit
      WHERE  ( aktivitaet = id_aktivitaet )
        .
    IF sy-subrc = 0.
*     insert into buffer
