CLASS zcl_cust_zcus_extsnunr DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS check_key
      IMPORTING
        !id_dienstleistung TYPE zcust_zcus_extsnunr-dienstleistung
        !id_meas           TYPE zcust_zcus_extsnunr-meas
        !id_snunr          TYPE zcust_zcus_extsnunr-snunr
      EXCEPTIONS
        not_found .
    CLASS-METHODS count
      RETURNING
    ...
    CLASS-METHODS get_detail
      ...
      RETURNING
        VALUE(rs_detail)   TYPE zcust_zcus_extsnunr
      EXCEPTIONS
        not_found .
    CLASS-METHODS get_detail_x
      IMPORTING
        ...
        !id_require        TYPE char1 DEFAULT ' '
      RETURNING
        VALUE(rs_detail_x) TYPE zcust_zcus_extsnunr_x
      EXCEPTIONS
        not_found .
    CLASS-METHODS get_list
      IMPORTING
        !id_require TYPE char1 DEFAULT ' '
      EXPORTING
        !et_list    TYPE z_t_zcus_extsnunr
      EXCEPTIONS
        not_found .
    CLASS-METHODS get_list_x
      IMPORTING
        ...
        !id_require TYPE char1 DEFAULT ' '
      EXPORTING
        !et_list_x  TYPE z_t_zcus_extsnunr_x
      EXCEPTIONS
        not_found .
    ...
    CLASS-METHODS transfer_to_buffer
      IMPORTING
        !it_list TYPE z_t_zcus_extsnunr .
PROTECTED SECTION.
PRIVATE SECTION.

  CONSTANTS mc_tabname TYPE tabname VALUE 'ZCUS_EXTSNUNR'.  "#EC NOTEXT
  CLASS-DATA mt_buffer TYPE z_t_zcus_extsnunr .
  CLASS-DATA ms_buffer_hl TYPE zcust_zcus_extsnunr .
...
CLASS ZCL_CUST_ZCUS_EXTSNUNR IMPLEMENTATION.
...
* headerline buffer usable?
  IF ( ms_buffer_hl-dienstleistung = id_dienstleistung ) AND
     ...
     ( ms_buffer_hl-snunr = id_snunr ) AND
...
*   entry found in headerline buffer
    rs_detail_x-zcust_zcus_extsnunr = ms_buffer_hl.
...
*   copy relevant set of entries and complete additional data
    LOOP AT mt_buffer ASSIGNING <ls_detail> FROM sy-tabix.
      IF ( <ls_detail>-dienstleistung <> id_dienstleistung ).
        EXIT.
      ELSE.
        ls_detail_x-zcust_zcus_extsnunr = <ls_detail>.
...
* read from database
  SELECT     *
    ...
    FROM     zcus_extsnunr.           "#EC CI_SGLSELECT "#EC CI_GENBUFF
