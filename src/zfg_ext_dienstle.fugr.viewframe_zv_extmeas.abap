*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZV_EXTMEAS
*---------------------------------------------------------------------*
FUNCTION viewframe_zv_extmeas          .
...
*----------------------------------------------------------------------*
    CALL FUNCTION 'VIEWPROC_ZV_EXTMEAS'
      EXPORTING
        fcode                     = read
      ...
      TABLES
        ...
        extract                   = zv_extmeas_extract
        total                     = zv_extmeas_total
        x_header                  = x_header
        x_namtab                  = x_namtab
...
*----------------------------------------------------------------------*
      DO.
        CALL FUNCTION 'VIEWPROC_ZV_EXTMEAS'
          ...
          IMPORTING
            ucomm                     = function
            update_required           = status_zv_extmeas-upd_flag
          TABLES
            excl_cua_funct            = excl_cua_funct
            extract                   = zv_extmeas_extract
            total                     = zv_extmeas_total
...
*----------------------------------------------------------------------*
      ...
      IF ( function EQ switch_to_show_mode OR
           ...
           function EQ end ) AND
status_zv_extmeas-upd_flag NE space.
        PERFORM beenden.
        CASE sy-subrc.
          WHEN 0.
            CALL FUNCTION 'VIEWPROC_ZV_EXTMEAS'
            ...
            CASE sy-subrc.
              WHEN 0.
                IF status_zv_extmeas-upd_flag EQ space. EXIT. ENDIF.
...
*----------------------------------------------------------------------*
      ELSEIF function EQ transport.
        IF status_zv_extmeas-upd_flag NE space.
