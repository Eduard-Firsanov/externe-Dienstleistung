*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZV_EXTSNUNR
*---------------------------------------------------------------------*
FUNCTION viewframe_zv_extsnunr         .
...
*----------------------------------------------------------------------*
    CALL FUNCTION 'VIEWPROC_ZV_EXTSNUNR'
      EXPORTING
        fcode                     = read
      ...
      TABLES
        ...
        extract                   = zv_extsnunr_extract
        total                     = zv_extsnunr_total
        x_header                  = x_header
        x_namtab                  = x_namtab
...
*----------------------------------------------------------------------*
      DO.
        CALL FUNCTION 'VIEWPROC_ZV_EXTSNUNR'
          ...
          IMPORTING
            ucomm                     = function
            update_required           = status_zv_extsnunr-upd_flag
          TABLES
            excl_cua_funct            = excl_cua_funct
            extract                   = zv_extsnunr_extract
            total                     = zv_extsnunr_total
...
*----------------------------------------------------------------------*
      ...
      IF ( function EQ switch_to_show_mode OR
           ...
           function EQ end ) AND
status_zv_extsnunr-upd_flag NE space.
        PERFORM beenden.
        CASE sy-subrc.
          WHEN 0.
            CALL FUNCTION 'VIEWPROC_ZV_EXTSNUNR'
            ...
            CASE sy-subrc.
              WHEN 0.
                IF status_zv_extsnunr-upd_flag EQ space. EXIT. ENDIF.
...
*----------------------------------------------------------------------*
      ELSEIF function EQ transport.
        IF status_zv_extsnunr-upd_flag NE space.
