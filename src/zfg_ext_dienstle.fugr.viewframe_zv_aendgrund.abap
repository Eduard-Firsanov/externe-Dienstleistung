*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZV_AENDGRUND
*---------------------------------------------------------------------*
FUNCTION viewframe_zv_aendgrund        .
...
*----------------------------------------------------------------------*
    CALL FUNCTION 'VIEWPROC_ZV_AENDGRUND'
      EXPORTING
        fcode                     = read
      ...
      TABLES
        ...
        extract                   = zv_aendgrund_extract
        total                     = zv_aendgrund_total
        x_header                  = x_header
        x_namtab                  = x_namtab
...
*----------------------------------------------------------------------*
      DO.
        CALL FUNCTION 'VIEWPROC_ZV_AENDGRUND'
          ...
          IMPORTING
            ucomm                     = function
            update_required           = status_zv_aendgrund-upd_flag
          TABLES
            excl_cua_funct            = excl_cua_funct
            extract                   = zv_aendgrund_extract
            total                     = zv_aendgrund_total
...
*----------------------------------------------------------------------*
      ...
      IF ( function EQ switch_to_show_mode OR
           ...
           function EQ end ) AND
status_zv_aendgrund-upd_flag NE space.
        PERFORM beenden.
        CASE sy-subrc.
          WHEN 0.
            CALL FUNCTION 'VIEWPROC_ZV_AENDGRUND'
            ...
            CASE sy-subrc.
              WHEN 0.
                IF status_zv_aendgrund-upd_flag EQ space. EXIT. ENDIF.
...
*----------------------------------------------------------------------*
      ELSEIF function EQ transport.
        IF status_zv_aendgrund-upd_flag NE space.
